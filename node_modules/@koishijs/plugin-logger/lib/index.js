...
// external/webui/plugins/logger/src/index.ts
var src_exports = {};
...
// external/webui/plugins/logger/src/file.ts
var import_promises = require("fs/promises");
var import_buffer = require("buffer");
var _FileWriter = class _FileWriter {
...
__name(_FileWriter, "FileWriter");
var FileWriter = _FileWriter;

// external/webui/plugins/logger/src/locales/zh-CN.yml
var zh_CN_default = { root: "存放输出日志的本地目录。", maxAge: "日志文件保存的最大天数。", maxSize: "单个日志文件的最大大小。" };

// external/webui/plugins/logger/src/index.ts
var import_meta = {};
var name = "logger";
var _LogProvider = class _LogProvider extends import_plugin_console.DataService {
  ...
  async get() {
    var _a;
    return (_a = this.getWriter()) == null ? void 0 : _a.read();
  }
};
__name(_LogProvider, "LogProvider");
...
async function apply(ctx, config) {
  ...
  for (const filename of await (0, import_promises2.readdir)(root)) {
    ...
    if (!capture)
      continue;
    (_b = files[_a = capture[1]]) != null ? _b : files[_a] = [];
  ...
  createFile(date, Math.max(...(_c = files[date]) != null ? _c : [0]) + 1);
  let buffer = [];
  const update = ctx.throttle(() => {
    var _a2;
    (_a2 = ctx.get("console")) == null ? void 0 : _a2.patch("logs", buffer);
    buffer = [];
  }, 100);
  ...
  const target = {
    ...
    record: (record) => {
      ...
      const scope = (_a2 = record.meta[import_koishi.Context.current]) == null ? void 0 : _a2.scope;
      if (loader && scope) {
      ...
      if (writer.size >= config.maxSize) {
        writer.close();
        const index = Math.max(...(_b2 = files[date2]) != null ? _b2 : [0]) + 1;
  ...
  ctx.on("dispose", () => {
    writer == null ? void 0 : writer.close();
    (0, import_koishi.remove)(import_koishi.Logger.targets, target);
    if (loader) {
      loader.prolog = [];
    }
  });
  for (const record of (loader == null ? void 0 : loader.prolog) || []) {
