...
import_core8.Schema.dynamic = /* @__PURE__ */ __name(function dynamic(name) {
  return import_core8.Schema.any().role("dynamic", { name });
}, "dynamic");
import_core8.Schema.filter = /* @__PURE__ */ __name(function filter() {
  return import_core8.Schema.any().role("filter");
}, "filter");
import_core8.Schema.computed = /* @__PURE__ */ __name(function computed(inner, options = {}) {
  return import_core8.Schema.union([
    import_core8.Schema.from(inner),
    import_core8.Schema.object({
      $switch: import_core8.Schema.object({
        branches: import_core8.Schema.array(import_core8.Schema.object({
          case: import_core8.Schema.any(),
          then: import_core8.Schema.from(inner)
        })),
        default: import_core8.Schema.from(inner)
      })
    }).hidden(),
    import_core8.Schema.any().hidden()
  ]).role("computed", options);
}, "computed");
import_core8.Schema.path = /* @__PURE__ */ __name(function path(options = {}) {
  return import_core8.Schema.string().role("path", options);
}, "path");
import_core8.Schema.prototype.computed = /* @__PURE__ */ __name(function computed2(options = {}) {
  return import_core8.Schema.computed(this, options).default(this.meta.default);
}, "computed");
var kSchemaOrder = Symbol("schema-order");
var SchemaService = class {
  constructor(ctx) {
    this.ctx = ctx;
    this.extend("intercept.http", import_core8.Schema.object({
      timeout: import_core8.Schema.natural().role("ms").description("等待连接建立的最长时间。"),
      proxyAgent: import_core8.Schema.string().description("使用的代理服务器地址。"),
      keepAlive: import_core8.Schema.boolean().description("是否保持连接。")
    }));
  }
  static {
  ...
  extend(name, schema, order = 0) {
    ...
    caller?.on("dispose", () => {
      (0, import_cosmokit9.remove)(target.list, schema);
      this.ctx.emit("internal/schema", name);
    });
  }
  get(name) {
    return this._data[name] ||= import_core8.Schema.intersect([]);
  }
  set(name, schema) {
    const caller = this[Context.current];
...
var logger4 = new import_core9.Logger("app");
var Permissions = class {
  constructor(ctx) {
    this.ctx = ctx;
    (0, import_cosmokit10.defineProperty)(this, Context.current, ctx);
  ...
  define(pattern, options) {
    ...
    return this.caller.effect(() => {
      ...
      return () => (0, import_cosmokit10.remove)(this.store, entry);
    });
  }
...
(0, import_cosmokit11.defineProperty)(import_core10.Bot, "filter", false);
(0, import_cosmokit11.defineProperty)(import_core10.Adapter, "filter", false);
var KoishiBot = class {
  static {
