...
var DatabaseService = class extends import_minato.Database {
  static {
    ...
    return (await Promise.all(this.ctx.bots.map((bot) => {
      ...
      const sessions = targets.map(({ id, guildId, locales }) => {
        const session = bot.session({
          type: "message",
          channel: { id, type: import_core3.Universal.Channel.Type.TEXT },
          guild: { id: guildId }
        });
...
var Processor = class {
  constructor(ctx) {
    this.ctx = ctx;
    (0, import_cosmokit4.defineProperty)(this, Context.current, ctx);
    ...
    ctx.component("i18n:time", (attrs, children, session) => {
      ...
      for (let index = 0; index < 3; index++) {
        const major = import_cosmokit4.Time[units[index]];
        const minor = import_cosmokit4.Time[units[index + 1]];
        if (ms >= major - minor / 2) {
          ms += minor / 2;
      ...
      return Math.round(ms / import_cosmokit4.Time.second) + " " + session.text("general.second");
    }, { session: true });
  ...
  _executeMatcher(session, matcher) {
    ...
    session.response = async () => {
      ...
      return import_core4.h.normalize(output, params.map((source) => source ? import_core4.h.parse(source) : ""));
    };
  }
...
// src/command/command.ts
var logger = new import_core5.Logger("command");
var Command = class extends Argv.CommandBase {
  ...
  set parent(parent) {
    if (this._parent === parent)
      return;
    if (this._parent) {
      (0, import_cosmokit5.remove)(this._parent.children, this);
    }
    this._parent = parent;
    if (parent) {
  ...
  shortcut(pattern, config = {}) {
    ...
    for (const key in config.options || {}) {
      content += ` --${(0, import_cosmokit5.camelize)(key)}`;
  ...
  before(callback, append = false) {
    ...
    this.caller.scope.disposables?.push(() => (0, import_cosmokit5.remove)(this._checkers, callback));
    return this;
  }
  action(callback, prepend = false) {
    ...
    this.caller.scope.disposables?.push(() => (0, import_cosmokit5.remove)(this._actions, callback));
    return this;
  }
  /** @deprecated */
  use(callback, ...args) {
  ...
  async execute(argv, fallback2 = Next.compose) {
    ...
    for (const validator of this._checkers) {
      ...
      if (!(0, import_cosmokit5.isNullable)(result))
        return result;
    }
...
${stack}`);
  ...
  dispose() {
    ...
    (0, import_cosmokit5.remove)(this.ctx.$commander._commandList, this);
    this.parent = null;
  }
  toJSON() {
    return {
...
((Command2) => {
  Command2.Config = import_core5.Schema.object({
    permissions: import_core5.Schema.array(String).role("perms").default(["authority:1"]).description("权限继承。"),
    dependencies: import_core5.Schema.array(String).role("perms").description("权限依赖。"),
    slash: import_core5.Schema.boolean().description("启用斜线指令功能。").default(true),
    checkUnknown: import_core5.Schema.boolean().description("是否检查未知选项。").default(false).hidden(),
    checkArgCount: import_core5.Schema.boolean().description("是否检查参数数量。").default(false).hidden(),
    showWarning: import_core5.Schema.boolean().description("是否显示警告。").default(true).hidden(),
    handleError: import_core5.Schema.union([import_core5.Schema.boolean(), import_core5.Schema.function()]).description("是否处理错误。").default(true).hidden()
  });
})(Command || (Command = {}));
  ...
  ctx.before("command/execute", async (argv) => {
    ...
    if (command.config.checkArgCount) {
      ...
      while (command._arguments[index]?.required) {
        ...
        if ((0, import_cosmokit6.isNullable)(source)) {
...
var Commander = class {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    this.config = config;
    (0, import_cosmokit7.defineProperty)(this, Context.current, ctx);
    ctx.plugin(validate);
    ...
    ctx.on("interaction/command", (session) => {
      if (session.event?.argv) {
        ...
        (0, import_cosmokit7.defineProperty)(session, "argv", ctx.bail("before-parse", session.content, session));
        if (!session.argv) {
    ...
    ctx.before("attach", (session) => {
      ...
      (0, import_cosmokit7.defineProperty)(session, "argv", ctx.bail("before-parse", content, session));
      if (!session.argv)
        return;
      session.argv.root = true;
      session.argv.session = session;
    ...
    ctx.schema.extend("command-option", import_core6.Schema.object({
      permissions: import_core6.Schema.array(String).role("perms").default(["authority:0"]).description("权限继承。"),
      dependencies: import_core6.Schema.array(String).role("perms").description("权限依赖。")
    }), 1e3);
    ctx.on("ready", () => {
      const bots = ctx.bots.filter((v) => v.status === import_core6.Universal.Status.ONLINE && v.updateCommands);
      bots.forEach((bot) => this.updateCommands(bot));
    });
    ctx.on("bot-status-updated", async (bot) => {
      if (bot.status !== import_core6.Universal.Status.ONLINE || !bot.updateCommands)
        return;
      this.updateCommands(bot);
    });
    this.domain("el", (source) => import_core6.h.parse(source), { greedy: true });
    this.domain("elements", (source) => import_core6.h.parse(source), { greedy: true });
    this.domain("string", (source) => import_core6.h.unescape(source));
    this.domain("text", (source) => import_core6.h.unescape(source), { greedy: true });
    this.domain("rawtext", (source) => (0, import_core6.h)("", import_core6.h.parse(source)).toString(true), { greedy: true });
    ...
    this.domain("date", (source, session) => {
      const timestamp = import_cosmokit7.Time.parseDate(source);
      if (+timestamp)
        return timestamp;
    ...
    this.domain("user", (source, session) => {
      ...
      const code = import_core6.h.from(source);
  ...
  _resolvePrefixes(session) {
    ...
    return result.map((source) => import_core6.h.escape(source)).sort().reverse();
  }
  available(session) {
...
// src/i18n.ts
var logger2 = new import_core7.Logger("i18n");
...
var I18n = class {
  ...
  *set(locale, prefix, value) {
    if (typeof value === "object" && value && !prefix.includes("@")) {
      ...
      if (!(0, import_cosmokit8.isNullable)(dict[path2]) && !locale.startsWith("$") && dict[path2] !== value) {
  ...
  _render(value, params, locale) {
    ...
    if (typeof value !== "string") {
      ...
      return [import_core7.h.text(render(value, params, locale))];
    }
    return import_core7.h.parse(value, params);
  }
  /** @deprecated */
  ...
  render(locales, paths, params) {
    ...
    return [import_core7.h.text(paths[0])];
  }
};
((I18n2) => {
  I18n2.Config = import_core7.Schema.object({
    locales: import_core7.Schema.array(String).role("table").default(["zh-CN", "en-US", "fr-FR", "ja-JP", "de-DE", "ru-RU"]).description("可用的语言列表。按照回退顺序排列。"),
    output: import_core7.Schema.union([
      import_core7.Schema.const("prefer-user").description("优先使用用户语言"),
      import_core7.Schema.const("prefer-channel").description("优先使用频道语言")
...
var logger3 = new import_core8.Logger("session");
...
var Session2 = class extends satori.Session {
  static {
    __name(this, "Session");
  }
  ...
  resolve(source, ...params) {
    ...
    if (!(0, import_minato2.isEvalExpr)(source))
      return source;
    return (0, import_minato2.executeEval)({ _: this }, source);
  }
  ...
  async sendQueued(content, delay) {
    const text = import_core8.h.normalize(content).join("");
    if (!text)
      return;
    if ((0, import_cosmokit9.isNullable)(delay)) {
  ...
  async withScope(scope, callback) {
    ...
    try {
      ...
      return import_core8.h.transform(result, {
        i18n: (params, children) => import_core8.h.i18n({
          ...params,
  ...
  i18n(path2, params = {}) {
    ...
    const paths = (0, import_cosmokit9.makeArray)(path2).map((path3) => this.resolveScope(path3));
    ...
    if (argv.tokens) {
      for (const arg of argv.tokens) {
        ...
        for (let i = 0; i < inters.length; ++i) {
          ...
          const transformed = await this.transform(import_core8.h.normalize(execution));
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) {
        if (typeof result === "string")
          return result;
        return (0, import_core8.h)(null, result).toString();
      }
...
import_core9.Schema.dynamic = /* @__PURE__ */ __name(function dynamic(name) {
  return import_core9.Schema.any().role("dynamic", { name });
}, "dynamic");
import_core9.Schema.filter = /* @__PURE__ */ __name(function filter() {
  return import_core9.Schema.any().role("filter");
}, "filter");
import_core9.Schema.computed = /* @__PURE__ */ __name(function computed(inner, options = {}) {
  return import_core9.Schema.union([
    import_core9.Schema.from(inner),
    import_core9.Schema.object({
      $switch: import_core9.Schema.object({
        branches: import_core9.Schema.array(import_core9.Schema.object({
          case: import_core9.Schema.any(),
          then: import_core9.Schema.from(inner)
        })),
        default: import_core9.Schema.from(inner)
      })
    }).hidden(),
    import_core9.Schema.any().hidden()
  ]).role("computed", options);
}, "computed");
import_core9.Schema.path = /* @__PURE__ */ __name(function path(options = {}) {
  return import_core9.Schema.string().role("path", options);
}, "path");
import_core9.Schema.prototype.computed = /* @__PURE__ */ __name(function computed2(options = {}) {
  return import_core9.Schema.computed(this, options).default(this.meta.default);
}, "computed");
...
var SchemaService = class {
  ...
  extend(name, schema, order = 0) {
    ...
    caller?.on("dispose", () => {
      (0, import_cosmokit10.remove)(target.list, schema);
      this.ctx.emit("internal/schema", name);
    });
  }
  get(name) {
    return this._data[name] ||= import_core9.Schema.intersect([]);
  }
  set(name, schema) {
    const caller = this[Context.current];
...
var logger4 = new import_core10.Logger("app");
var Permissions = class {
  constructor(ctx) {
    this.ctx = ctx;
    (0, import_cosmokit11.defineProperty)(this, Context.current, ctx);
  ...
  define(pattern, options) {
    ...
    return this.caller.effect(() => {
      ...
      return () => (0, import_cosmokit11.remove)(this.store, entry);
    });
  }
...
var Context = class extends satori2.Context {
  static {
    __name(this, "Context");
  }
...
Context.Config.list.push(import_core11.Schema.object({
  request: import_core11.Quester.Config
}));
var Service2 = class extends satori2.Service {
  static {
    __name(this, "Service");
  }
  [satori2.Service.setup]() {
