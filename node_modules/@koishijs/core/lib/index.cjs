...
__export(src_exports, {
  Adapter: () => import_core14.Adapter,
  App: () => Context,
  Argv: () => Argv,
  Bot: () => import_core14.Bot,
  Channel: () => Channel,
  Command: () => Command,
  Commander: () => Commander,
  Context: () => Context,
  DatabaseService: () => DatabaseService,
  Element: () => import_core14.Element,
  FilterService: () => FilterService,
  I18n: () => I18n,
  Logger: () => import_core14.Logger,
  MessageEncoder: () => import_core14.MessageEncoder,
  Messenger: () => import_core14.Messenger,
  Next: () => Next,
  Permissions: () => Permissions,
  Processor: () => Processor,
  Quester: () => import_core14.Quester,
  Satori: () => import_core14.Satori,
  Schema: () => import_core14.Schema,
  SchemaService: () => SchemaService,
  Service: () => Service2,
  Session: () => Session2,
  SessionError: () => SessionError,
  SharedCache: () => SharedCache,
  Universal: () => import_core14.Universal,
  User: () => User,
...
__reExport(src_exports, require("@minatojs/core"), module.exports);

// koishi/packages/core/src/bot.ts
var import_utils = require("@koishijs/utils");
var import_cosmokit = require("cosmokit");
var import_core = require("@satorijs/core");
(0, import_cosmokit.defineProperty)(import_core.Bot, "filter", false);
(0, import_cosmokit.defineProperty)(import_core.Adapter, "filter", false);
import_core.Bot.prototype.getGuildMemberMap = /* @__PURE__ */ __name(async function getGuildMemberMap(guildId) {
  const result = {};
...
import_core.Bot.prototype.broadcast = /* @__PURE__ */ __name(async function broadcast(channels, content, delay = this.ctx.root.config.delay.broadcast) {
  const ids = [];
  for (let index = 0; index < channels.length; index++) {
    if (index && delay)
      await (0, import_utils.sleep)(delay);
    try {
...
var import_cosmokit13 = require("cosmokit");
var import_core13 = require("@satorijs/core");
var satori2 = __toESM(require("@satorijs/core"));

// koishi/packages/core/src/filter.ts
...
var _FilterService = class _FilterService {
  constructor(app) {
    this.app = app;
    (0, import_cosmokit2.defineProperty)(this, Context.current, app);
    app.filter = () => true;
  ...
  union(arg) {
    ...
    return caller.extend({ filter: (s) => caller.filter(s) || filter2(s) });
  }
  intersect(arg) {
    const caller = this.caller;
    const filter2 = typeof arg === "function" ? arg : arg.filter;
...
((Argv2) => {
  ...
  const _Tokenizer = class _Tokenizer {
    ...
    parseToken(source, stopReg = "$") {
      ...
      stopReg += `|${Object.keys({ ...this.bracs, ...bracs }).map(import_utils2.escapeRegExp).join("|")}`;
    ...
    parse(source, terminator = "") {
      const tokens = [];
      source = import_core2.h.parse(source).map((el) => {
        return el.type === "text" ? el.toString() : whitespace.escape(el.toString());
      }).join("");
      let rest = source, term = "";
      const stopReg = `\\s+|[${(0, import_utils2.escapeRegExp)(terminator)}]|$`;
  ...
  const _CommandBase = class _CommandBase {
    ...
    _createOption(name, def, config) {
      ...
      const param = (0, import_cosmokit3.paramCase)(name);
      let syntax = cap[1] || "--" + param;
      const bracket = cap[2] || "";
      const desc = cap[3].trim();
      const aliases = (_a = config.aliases) != null ? _a : [];
      const symbols = (_b = config.symbols) != null ? _b : [];
      for (let param2 of syntax.trim().split(",")) {
        param2 = param2.trimStart();
        const name2 = param2.replace(/^-+/, "");
        if (!name2 || !param2.startsWith("-")) {
          symbols.push(import_core2.h.escape(param2));
        } else {
          aliases.push(name2);
        }
      }
      ...
      const option = (_c = this._options)[name] || (_c[name] = {
        ...declList[0],
        ...config,
        name,
        values: {},
    ...
    parse(argv, terminator, args = [], options = {}) {
      ...
      let lastArgDecl;
      while (!argv.error && ((_a = argv.tokens) == null ? void 0 : _a.length)) {
        ...
        if (!quoted && (option = this._symbolicOptions[content])) {
          names = [(0, import_cosmokit3.paramCase)(option.name)];
        } else {
          ...
          if (i > 1 && name.startsWith("no-") && !this._namedOptions[name]) {
            options[(0, import_cosmokit3.camelCase)(name.slice(3))] = false;
            continue;
          }
          names = i > 1 ? [name] : name;
        ...
        if (!param) {
          ...
          if (this.ctx.$commander.resolveDomain(type).greedy) {
            ...
            if (!isValued && argv.tokens.length && (type || ((_b = argv.tokens[0]) == null ? void 0 : _b.content) !== "-")) {
        ...
        for (let j = 0; j < names.length; j++) {
          ...
          const key = optDecl ? optDecl.name : (0, import_cosmokit3.camelCase)(name);
  ...
  __name(_CommandBase, "CommandBase");
  let CommandBase = _CommandBase;
  Argv2.CommandBase = _CommandBase;
})(Argv || (Argv = {}));
...
var import_core3 = require("@minatojs/core");
...
var _DatabaseService = class _DatabaseService extends import_core3.Database {
  constructor(app) {
    super();
    this.app = app;
    (0, import_cosmokit4.defineProperty)(this, Context.current, app);
    this.extend("user", {
      id: "unsigned(8)",
    ...
    app.on("login-added", ({ platform }) => {
      if (platform in this.tables.user.fields)
        return;
  ...
  getSelfIds(platforms) {
    ...
    for (const bot of this.app.bots) {
      if (platforms && !platforms.includes(bot.platform))
        continue;
      (selfIdMap[_a = bot.platform] || (selfIdMap[_a] = [])).push(bot.selfId);
    }
    return selfIdMap;
  }
  ...
  async broadcast(...args) {
    ...
    for (const channel of data) {
      ...
      if (!forced && flag & 4 /* silent */)
        continue;
      ((_a = assignMap[platform] || (assignMap[platform] = {}))[assignee] || (_a[assignee] = [])).push(channel);
    }
    ...
    return (await Promise.all(this.app.bots.map((bot) => {
      var _a2;
      const targets = (_a2 = assignMap[bot.platform]) == null ? void 0 : _a2[bot.selfId];
      if (!targets)
        return Promise.resolve([]);
      const sessions = targets.map(({ id, guildId, locales }) => {
        const session = bot.session({
          type: "message",
          channel: { id, type: import_core4.Universal.Channel.Type.TEXT },
          guild: { id: guildId }
        });
...
var defineDriver = /* @__PURE__ */ __name((constructor, schema, prepare) => ({
  ...
  async apply(ctx, config) {
    config = { ...config };
    await (prepare == null ? void 0 : prepare(ctx, config));
...
var _SessionError = class _SessionError extends Error {
  constructor(path2, param) {
    super((0, import_utils3.makeArray)(path2)[0]);
    this.path = path2;
    this.param = param;
  }
};
...
var _Processor = class _Processor {
  constructor(ctx) {
    ...
    ctx.component("random", async (attrs, children) => {
      return import_utils3.Random.pick(children);
    });
    ...
    ctx.component("i18n:time", (attrs, children, session) => {
      ...
      for (let index = 0; index < 3; index++) {
        const major = import_cosmokit5.Time[units[index]];
        const minor = import_cosmokit5.Time[units[index + 1]];
        if (ms >= major - minor / 2) {
          ms += minor / 2;
      ...
      return Math.round(ms / import_cosmokit5.Time.second) + " " + session.text("general.second");
    }, { session: true });
  ...
  _executeMatcher(session, matcher) {
    ...
    if (quote == null ? void 0 : quote.content)
    ...
    session.response = async () => {
      ...
      return import_core5.h.normalize(output, params.map((source) => source ? import_core5.h.parse(source) : ""));
    };
  }
  ...
  async _handleMessage(session) {
    ...
    const next = /* @__PURE__ */ __name(async (callback) => {
      ...
      try {
        ...
        const stack = (0, import_utils3.coerce)(error);
...
var _keyMap;
var _SharedCache = class _SharedCache {
  constructor() {
...
_keyMap = new WeakMap();
__name(_SharedCache, "SharedCache");
var SharedCache = _SharedCache;

// koishi/packages/core/src/command/command.ts
var logger = new import_core6.Logger("command");
var _Command = class _Command extends Argv.CommandBase {
  children = [];
  _parent = null;
  ...
  constructor(name, decl, ctx, config) {
    ...
    (_c = (_b = this.config).permissions) != null ? _c : _b.permissions = [`authority:${(_a = config == null ? void 0 : config.authority) != null ? _a : 1}`];
  ...
  set parent(parent) {
    if (this._parent === parent)
      return;
    if (this._parent) {
      (0, import_cosmokit6.remove)(this._parent.children, this);
    }
    this._parent = parent;
    if (parent) {
  ...
  shortcut(pattern, config = {}) {
    ...
    for (const key in config.options || {}) {
      content += ` --${(0, import_cosmokit6.camelize)(key)}`;
  ...
  option(name, ...args) {
    ...
    const config = { ...args[0] };
    (_b = config.permissions) != null ? _b : config.permissions = [`authority:${(_a = config.authority) != null ? _a : 0}`];
  ...
  before(callback, append = false) {
    ...
    (_a = this.caller.scope.disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit6.remove)(this._checkers, callback));
    return this;
  }
  action(callback, prepend = false) {
    ...
    (_a = this.caller.scope.disposables) == null ? void 0 : _a.push(() => (0, import_cosmokit6.remove)(this._actions, callback));
    return this;
  }
  /** @deprecated */
  use(callback, ...args) {
  ...
  async execute(argv, fallback2 = Next.compose) {
    ...
    for (const validator of this._checkers) {
      ...
      if (!(0, import_cosmokit6.isNullable)(result))
        return result;
    }
    ...
    try {
      ...
      const stack = (0, import_utils4.coerce)(error2);
...
${stack}`);
  ...
  dispose() {
    ...
    (0, import_cosmokit6.remove)(this.ctx.$commander._commandList, this);
    this.parent = null;
  }
  toJSON() {
    return {
...
((Command2) => {
  Command2.Config = import_core6.Schema.object({
    permissions: import_core6.Schema.array(String).role("perms").default(["authority:1"]).description("权限继承。"),
    dependencies: import_core6.Schema.array(String).role("perms").description("权限依赖。"),
    slash: import_core6.Schema.boolean().description("启用斜线指令功能。").default(true),
    checkUnknown: import_core6.Schema.boolean().description("是否检查未知选项。").default(false).hidden(),
    checkArgCount: import_core6.Schema.boolean().description("是否检查参数数量。").default(false).hidden(),
    showWarning: import_core6.Schema.boolean().description("是否显示警告。").default(true).hidden(),
    handleError: import_core6.Schema.union([import_core6.Schema.boolean(), import_core6.Schema.function()]).description("是否处理错误。").default(true).hidden()
  });
})(Command || (Command = {}));
...
function validate(ctx) {
  ctx.permissions.define("command:(name)", {
    depends: ({ name }) => {
      ...
      if (!command)
        return;
      const depends = [...(_a = command.config.dependencies) != null ? _a : []];
      if (command.parent)
    ...
    inherits: ({ name }) => {
      var _a;
      return (_a = ctx.$commander.get(name)) == null ? void 0 : _a.config.permissions;
    },
    list: () => {
  ...
  ctx.permissions.define("command:(name):option:(name2)", {
    depends: ({ name, name2 }) => {
      var _a, _b;
      return (_b = (_a = ctx.$commander.get(name)) == null ? void 0 : _a._options[name2]) == null ? void 0 : _b.dependencies;
    },
    inherits: ({ name, name2 }) => {
      var _a, _b;
      return (_b = (_a = ctx.$commander.get(name)) == null ? void 0 : _a._options[name2]) == null ? void 0 : _b.permissions;
    },
    list: () => {
  ...
  ctx.before("command/execute", async (argv) => {
    ...
    if (command.config.checkArgCount) {
      ...
      while ((_a = command._arguments[index]) == null ? void 0 : _a.required) {
        ...
        if ((0, import_cosmokit7.isNullable)(source)) {
...
var _Commander = class _Commander {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    this.config = config;
    (0, import_cosmokit8.defineProperty)(this, Context.current, ctx);
    ctx.plugin(validate);
    ...
    ctx.on("interaction/command", (session) => {
      var _a;
      if ((_a = session.event) == null ? void 0 : _a.argv) {
        ...
        (0, import_cosmokit8.defineProperty)(session, "argv", ctx.bail("before-parse", session.content, session));
        if (!session.argv) {
    ...
    ctx.before("attach", (session) => {
      ...
      (0, import_cosmokit8.defineProperty)(session, "argv", ctx.bail("before-parse", content, session));
      if (!session.argv)
        return;
      session.argv.root = true;
      session.argv.session = session;
    ...
    ctx.middleware((session, next) => {
      ...
      if ((argv == null ? void 0 : argv.command) || !isDirect && !prefix && !appel)
        return next();
      const content = session.stripped.content.slice((prefix != null ? prefix : "").length);
    ...
    ctx.schema.extend("command-option", import_core7.Schema.object({
      permissions: import_core7.Schema.array(String).role("perms").default(["authority:0"]).description("权限继承。"),
      dependencies: import_core7.Schema.array(String).role("perms").description("权限依赖。")
    }), 1e3);
    ctx.on("ready", () => {
      const bots = ctx.bots.filter((v) => v.status === import_core7.Universal.Status.ONLINE && v.updateCommands);
      bots.forEach((bot) => this.updateCommands(bot));
    });
    ctx.on("bot-status-updated", async (bot) => {
      if (bot.status !== import_core7.Universal.Status.ONLINE || !bot.updateCommands)
        return;
      this.updateCommands(bot);
    });
    this.domain("el", (source) => import_core7.h.parse(source), { greedy: true });
    this.domain("elements", (source) => import_core7.h.parse(source), { greedy: true });
    this.domain("string", (source) => import_core7.h.unescape(source));
    this.domain("text", (source) => import_core7.h.unescape(source), { greedy: true });
    this.domain("rawtext", (source) => (0, import_core7.h)("", import_core7.h.parse(source)).toString(true), { greedy: true });
    ...
    this.domain("date", (source, session) => {
      const timestamp = import_cosmokit8.Time.parseDate(source);
      if (+timestamp)
        return timestamp;
    ...
    this.domain("user", (source, session) => {
      ...
      const code = import_core7.h.from(source);
  ...
  get(name, session) {
    return this._commandList.find((cmd) => {
      ...
      return alias && ((_a = session == null ? void 0 : session.resolve(alias.filter)) != null ? _a : true);
    });
  }
  updateCommands(bot) {
  ...
  _resolvePrefixes(session) {
    ...
    return result.map((source) => import_core7.h.escape(source)).sort().reverse();
  }
  available(session) {
  ...
  inferCommand(argv) {
    ...
    if (argv.root && !isDirect && stripped.prefix === null && !stripped.appel)
      return;
    const segments = [];
  ...
  command(def, ...args) {
    ...
    if (root)
      caller.collect(`command <${root.name}>`, () => root.dispose());
    return parent;
  }
  ...
  resolveDomain(type) {
    ...
    return (_a = this.ctx.get(`domain:${type}`)) != null ? _a : {};
  }
...
// koishi/packages/core/src/i18n.ts
var logger2 = new import_core8.Logger("i18n");
...
var _I18n = class _I18n {
  ...
  compare(expect, actual, options = {}) {
    ...
    const threshold = (_a = options.minSimilarity) != null ? _a : this.ctx.root.config.minSimilarity;
  ...
  get(key, locales = []) {
    ...
    for (const locale of this.fallback(locales)) {
      const value = (_a = this._data[locale]) == null ? void 0 : _a[key];
      if (value)
        result[locale] = value;
    }
    return result;
  ...
  *set(locale, prefix, value) {
    if (typeof value === "object" && value && !prefix.includes("@")) {
      ...
      if (!(0, import_cosmokit9.isNullable)(dict[path2]) && !locale.startsWith("$") && dict[path2] !== value) {
  ...
  define(locale, ...args) {
    ...
    const dict = (_a = this._data)[locale] || (_a[locale] = {});
  ...
  _render(value, params, locale) {
    ...
    if (typeof value !== "string") {
      ...
      return [import_core8.h.text(render(value, params, locale))];
    }
    return import_core8.h.parse(value, params);
  }
  /** @deprecated */
  ...
  render(locales, paths, params) {
    ...
    for (const path2 of paths) {
      for (const locale of locales) {
        for (const key of ["$" + locale, locale]) {
          const value = (_a = this._data[key]) == null ? void 0 : _a[path2];
    ...
    return [import_core8.h.text(paths[0])];
  }
};
__name(_I18n, "I18n");
var I18n = _I18n;
((I18n2) => {
  I18n2.Config = import_core8.Schema.object({
    locales: import_core8.Schema.array(String).role("table").default(["zh-CN", "en-US", "fr-FR", "ja-JP", "de-DE", "ru-RU"]).description("可用的语言列表。按照回退顺序排列。"),
    output: import_core8.Schema.union([
      import_core8.Schema.const("prefer-user").description("优先使用用户语言"),
      import_core8.Schema.const("prefer-channel").description("优先使用频道语言")
...
var import_core10 = require("@minatojs/core");
var satori = __toESM(require("@satorijs/core"));
var logger3 = new import_core9.Logger("session");
...
var _Session = class _Session extends satori.Session {
  argv;
  user;
  channel;
  guild;
  ...
  resolve(source, ...params) {
    ...
    if (!(0, import_core10.isEvalExpr)(source))
      return source;
    return (0, import_core10.executeEval)({ _: this }, source);
  }
  _stripNickname(content) {
    var _a;
    if (content.startsWith("@"))
      content = content.slice(1);
    for (const nickname of (_a = this.resolve(this.app.config.nickname)) != null ? _a : []) {
  ...
  get stripped() {
    ...
    while (((_a = elements[0]) == null ? void 0 : _a.type) === "at") {
      const { attrs } = elements.shift();
      if (attrs.id === this.selfId) {
        atSelf = appel = true;
      }
      if (!((_c = (_b = this.quote) == null ? void 0 : _b.user) == null ? void 0 : _c.id) || this.quote.user.id !== attrs.id) {
        hasAt = true;
      }
      if (((_d = elements[0]) == null ? void 0 : _d.type) === "text" && !elements[0].attrs.content.trim()) {
        elements.shift();
      }
    }
  ...
  async sendQueued(content, delay) {
    const text = import_core9.h.normalize(content).join("");
    if (!text)
      return;
    if ((0, import_cosmokit10.isNullable)(delay)) {
  ...
  async _observeChannelLike(channelId, fields = []) {
    ...
    if (cache) {
      ...
      cache = (0, import_utils5.observe)(data, async (diff) => {
        if (data["$detached"])
          return;
  ...
  async observeUser(fields) {
    ...
    if ((_a = this.author) == null ? void 0 : _a["anonymous"]) {
      ...
      fallback2.authority = this.resolve(this.app.config.autoAuthorize);
      const user = (0, import_utils5.observe)(fallback2, () => Promise.resolve());
  ...
  async withScope(scope, callback) {
    ...
    try {
      ...
      return import_core9.h.transform(result, {
        i18n: (params, children) => import_core9.h.i18n({
          ...params,
  ...
  i18n(path2, params = {}) {
    var _a, _b, _c, _d;
    const locales = [
      ...((_a = this.channel) == null ? void 0 : _a.locales) || [],
      ...((_b = this.guild) == null ? void 0 : _b.locales) || []
    ];
    if (this.app.config.i18n.output === "prefer-user") {
      locales.unshift(...((_c = this.user) == null ? void 0 : _c.locales) || []);
    } else {
      locales.push(...((_d = this.user) == null ? void 0 : _d.locales) || []);
    }
    locales.unshift(...this.locales || []);
    const paths = (0, import_cosmokit10.makeArray)(path2).map((path3) => this.resolveScope(path3));
    ...
    if (argv.tokens) {
      for (const arg of argv.tokens) {
        ...
        for (let i = 0; i < inters.length; ++i) {
          ...
          const transformed = await this.transform(import_core9.h.normalize(execution));
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) {
        if (typeof result === "string")
          return result;
        return (0, import_core9.h)(null, result).toString();
      }
  ...
  prompt(...args) {
    ...
    const callback = typeof args[0] === "function" ? args.shift() : (session) => {
      ...
      if (((_a2 = elements[0]) == null ? void 0 : _a2.type) === "at" && elements[0].attrs.id === session.selfId) {
        elements.shift();
      }
      return elements.join("").trim();
    };
    const options = typeof args[0] === "number" ? { timeout: args[0] } : (_a = args[0]) != null ? _a : {};
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        dispose();
        resolve(void 0);
      }, (_a2 = options.timeout) != null ? _a2 : this.app.config.delay.prompt);
    });
  }
...
__publicField(_Session, "shadow", Symbol.for("session.shadow"));
var Session2 = _Session;

// koishi/packages/core/src/schema.ts
var import_cosmokit11 = require("cosmokit");
var import_core11 = require("@satorijs/core");
import_core11.Schema.dynamic = /* @__PURE__ */ __name(function dynamic(name) {
  return import_core11.Schema.any().role("dynamic", { name });
}, "dynamic");
import_core11.Schema.filter = /* @__PURE__ */ __name(function filter() {
  return import_core11.Schema.any().role("filter");
}, "filter");
import_core11.Schema.computed = /* @__PURE__ */ __name(function computed(inner, options = {}) {
  return import_core11.Schema.union([
    import_core11.Schema.from(inner),
    import_core11.Schema.object({
      $switch: import_core11.Schema.object({
        branches: import_core11.Schema.array(import_core11.Schema.object({
          case: import_core11.Schema.any(),
          then: import_core11.Schema.from(inner)
        })),
        default: import_core11.Schema.from(inner)
      })
    }).hidden(),
    import_core11.Schema.any().hidden()
  ]).role("computed", options);
}, "computed");
import_core11.Schema.path = /* @__PURE__ */ __name(function path(options = {}) {
  return import_core11.Schema.string().role("path", options);
}, "path");
import_core11.Schema.prototype.computed = /* @__PURE__ */ __name(function computed2(options = {}) {
  return import_core11.Schema.computed(this, options).default(this.meta.default);
}, "computed");
var kSchemaOrder = Symbol("schema-order");
var _SchemaService = class _SchemaService {
  constructor(ctx) {
    this.ctx = ctx;
  }
  ...
  get(name) {
    var _a;
    return (_a = this._data)[name] || (_a[name] = import_core11.Schema.intersect([]));
  }
  set(name, schema) {
    const caller = this[Context.current];
...
__name(_SchemaService, "SchemaService");
...
var logger4 = new import_core12.Logger("app");
var _Permissions = class _Permissions {
  constructor(ctx) {
    this.ctx = ctx;
    (0, import_cosmokit12.defineProperty)(this, Context.current, ctx);
    ...
    this.provide("(name)", ({ name }, session) => {
      var _a;
      return (_a = session.bot) == null ? void 0 : _a.checkPermission(name, session);
    });
    this.provide("(name)", ({ name }, session) => {
      var _a, _b, _c, _d, _e;
      return ((_a = session.permissions) == null ? void 0 : _a.includes(name)) || ((_c = (_b = session.user) == null ? void 0 : _b.permissions) == null ? void 0 : _c.includes(name)) || ((_e = (_d = session.channel) == null ? void 0 : _d.permissions) == null ? void 0 : _e.includes(name));
    });
  }
  store = [];
  get caller() {
  ...
  define(pattern, options) {
    ...
    if (!pattern.includes("("))
      entry.list || (entry.list = () => [pattern]);
    return this.caller.effect(() => {
      this.store.push(entry);
      return () => (0, import_cosmokit12.remove)(this.store, entry);
    });
  }
  ...
  async test(names, session = {}, cache = /* @__PURE__ */ new Map()) {
    session = session[Session2.shadow] || session;
...
// koishi/packages/core/src/context.ts
var import_core14 = require("@satorijs/core");
var import_cordis = require("cordis");
var _Service = class _Service extends satori2.Service {
};
__name(_Service, "Service");
var Service2 = _Service;
var _Context = class _Context extends satori2.Context {
  constructor(config = {}) {
    super(config);
    ...
    this.provide("database", void 0, true);
...
__publicField(_Context, "Session", Session2);
var Context = _Context;
Session2.prototype[Context.filter] = function(ctx) {
  return ctx.filter(this);
};
((Context3) => {
  Context3.Config = import_core13.Schema.intersect([
    import_core13.Schema.object({})
  ]);
})(Context || (Context = {}));
(0, import_cosmokit13.defineProperty)(Context.Config, "Basic", import_core13.Schema.object({
  prefix: import_core13.Schema.array(import_core13.Schema.string().default("")).default([""]).role("table").computed().description("指令前缀字符构成的数组。将被用于指令的匹配。"),
  nickname: import_core13.Schema.array(String).role("table").computed().description("机器人昵称构成的数组。将被用于指令的匹配。"),
  autoAssign: import_core13.Schema.boolean().default(true).computed().description("当获取不到频道数据时，是否使用接受者作为受理人。"),
  autoAuthorize: import_core13.Schema.natural().default(1).computed().description("当获取不到用户数据时默认使用的权限等级。"),
  minSimilarity: import_core13.Schema.percent().default(1).description("用于模糊匹配的相似系数，应该是一个 0 到 1 之间的数值。数值越高，模糊匹配越严格。设置为 1 可以完全禁用模糊匹配。")
}).description("基础设置"));
(0, import_cosmokit13.defineProperty)(Context.Config, "I18n", I18n.Config);
(0, import_cosmokit13.defineProperty)(Context.Config, "Delay", import_core13.Schema.object({
  character: import_core13.Schema.natural().role("ms").default(0).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按前一条消息的字数计算。"),
  message: import_core13.Schema.natural().role("ms").default(0.1 * import_cosmokit13.Time.second).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按固定值计算。"),
  cancel: import_core13.Schema.natural().role("ms").default(0).description("调用 `session.cancelQueued()` 时默认的延迟。"),
  broadcast: import_core13.Schema.natural().role("ms").default(0.5 * import_cosmokit13.Time.second).description("调用 `bot.broadcast()` 时默认的延迟。"),
  prompt: import_core13.Schema.natural().role("ms").default(import_cosmokit13.Time.minute).description("调用 `session.prompt()` 时默认的等待时间。")
}));
(0, import_cosmokit13.defineProperty)(Context.Config, "Advanced", import_core13.Schema.object({
  maxListeners: import_core13.Schema.natural().default(64).description("每种监听器的最大数量。如果超过这个数量，Koishi 会认定为发生了内存泄漏，将产生一个警告。")
}).description("高级设置"));
Context.Config.list.push(Context.Config.Basic);
Context.Config.list.push(import_core13.Schema.object({
  i18n: I18n.Config
}));
Context.Config.list.push(import_core13.Schema.object({
...
Context.Config.list.push(import_core13.Schema.object({
  request: import_core13.Quester.Config
}));
