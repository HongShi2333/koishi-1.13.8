...
import { Adapter, Bot } from "@satorijs/core";
...
import { defineProperty as defineProperty7, Time as Time3 } from "cosmokit";
import { Quester, Schema as Schema6 } from "@satorijs/core";
import * as satori2 from "@satorijs/core";
...
var FilterService = class {
  constructor(app) {
    this.app = app;
    defineProperty2(this, Context.current, app);
    app.filter = () => true;
...
import { defineProperty as defineProperty5, Time as Time2 } from "cosmokit";
import { h as h3, Schema as Schema3, Universal as Universal3 } from "@satorijs/core";
...
import { Logger, Schema as Schema2 } from "@satorijs/core";
...
import { defineProperty as defineProperty4, Time } from "cosmokit";
...
var DatabaseService = class extends Database {
  constructor(app) {
    super();
    this.app = app;
    defineProperty3(this, Context.current, app);
    this.extend("user", {
      id: "unsigned(8)",
...
((Command2) => {
  Command2.Config = Schema2.object({
    permissions: Schema2.array(String).role("perms").default(["authority:1"]).description("权限继承。"),
    dependencies: Schema2.array(String).role("perms").description("权限依赖。"),
    slash: Schema2.boolean().description("启用斜线指令功能。").default(true),
    checkUnknown: Schema2.boolean().description("是否检查未知选项。").default(false).hidden(),
    checkArgCount: Schema2.boolean().description("是否检查参数数量。").default(false).hidden(),
    showWarning: Schema2.boolean().description("是否显示警告。").default(true).hidden(),
    handleError: Schema2.union([Schema2.boolean(), Schema2.function()]).description("是否处理错误。").default(true).hidden()
  });
})(Command || (Command = {}));
  ...
  constructor(ctx, config = {}) {
    ...
    ctx.on("interaction/command", (session) => {
      if (session.event?.argv) {
        ...
        defineProperty5(session, "argv", ctx.bail("before-parse", session.content, session));
        if (!session.argv) {
    ...
    ctx.before("attach", (session) => {
      ...
      defineProperty5(session, "argv", ctx.bail("before-parse", content, session));
      if (!session.argv)
        return;
      session.argv.root = true;
      session.argv.session = session;
    ...
    ctx.schema.extend("command-option", Schema3.object({
      permissions: Schema3.array(String).role("perms").default(["authority:0"]).description("权限继承。"),
      dependencies: Schema3.array(String).role("perms").description("权限依赖。")
    }), 1e3);
    ctx.on("ready", () => {
...
import { h as h4, Logger as Logger2, Schema as Schema4 } from "@satorijs/core";
...
((I18n2) => {
  I18n2.Config = Schema4.object({
    locales: Schema4.array(String).role("table").default(["zh-CN", "en-US", "fr-FR", "ja-JP", "de-DE", "ru-RU"]).description("可用的语言列表。按照回退顺序排列。"),
    output: Schema4.union([
      Schema4.const("prefer-user").description("优先使用用户语言"),
      Schema4.const("prefer-channel").description("优先使用频道语言")
...
import { executeEval, isEvalExpr } from "@minatojs/core";
...
import { remove as remove2 } from "cosmokit";
import { Schema as Schema5 } from "@satorijs/core";
Schema5.dynamic = /* @__PURE__ */ __name(function dynamic(name) {
  return Schema5.any().role("dynamic", { name });
}, "dynamic");
Schema5.filter = /* @__PURE__ */ __name(function filter() {
  return Schema5.any().role("filter");
}, "filter");
Schema5.computed = /* @__PURE__ */ __name(function computed(inner, options = {}) {
  return Schema5.union([
    Schema5.from(inner),
    Schema5.object({
      $switch: Schema5.object({
        branches: Schema5.array(Schema5.object({
          case: Schema5.any(),
          then: Schema5.from(inner)
        })),
        default: Schema5.from(inner)
      })
    }).hidden(),
    Schema5.any().hidden()
  ]).role("computed", options);
}, "computed");
Schema5.path = /* @__PURE__ */ __name(function path(options = {}) {
  return Schema5.string().role("path", options);
}, "path");
Schema5.prototype.computed = /* @__PURE__ */ __name(function computed2(options = {}) {
  return Schema5.computed(this, options).default(this.meta.default);
}, "computed");
...
var SchemaService = class {
  ...
  extend(name, schema, order = 0) {
    ...
    caller?.on("dispose", () => {
      remove2(target.list, schema);
      this.ctx.emit("internal/schema", name);
    });
  }
  get(name) {
    return this._data[name] ||= Schema5.intersect([]);
  }
  set(name, schema) {
    const caller = this[Context.current];
...
import { defineProperty as defineProperty6, remove as remove3 } from "cosmokit";
var logger4 = new Logger4("app");
var Permissions = class {
  constructor(ctx) {
    this.ctx = ctx;
    defineProperty6(this, Context.current, ctx);
  ...
  define(pattern, options) {
    ...
    return this.caller.effect(() => {
      ...
      return () => remove3(this.store, entry);
    });
  }
...
// koishi/packages/core/src/context.ts
import { Adapter as Adapter2, Bot as Bot3, Element, h as h6, Logger as Logger5, MessageEncoder, Messenger, Quester as Quester2, Satori, Schema as Schema7, segment, Universal as Universal5, z } from "@satorijs/core";
...
((Context3) => {
  Context3.Config = Schema6.intersect([
    Schema6.object({})
  ]);
})(Context || (Context = {}));
defineProperty7(Context.Config, "Basic", Schema6.object({
  prefix: Schema6.array(Schema6.string().default("")).default([""]).role("table").computed().description("指令前缀字符构成的数组。将被用于指令的匹配。"),
  nickname: Schema6.array(String).role("table").computed().description("机器人昵称构成的数组。将被用于指令的匹配。"),
  autoAssign: Schema6.boolean().default(true).computed().description("当获取不到频道数据时，是否使用接受者作为受理人。"),
  autoAuthorize: Schema6.natural().default(1).computed().description("当获取不到用户数据时默认使用的权限等级。"),
  minSimilarity: Schema6.percent().default(1).description("用于模糊匹配的相似系数，应该是一个 0 到 1 之间的数值。数值越高，模糊匹配越严格。设置为 1 可以完全禁用模糊匹配。")
}).description("基础设置"));
defineProperty7(Context.Config, "I18n", I18n.Config);
defineProperty7(Context.Config, "Delay", Schema6.object({
  character: Schema6.natural().role("ms").default(0).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按前一条消息的字数计算。"),
  message: Schema6.natural().role("ms").default(0.1 * Time3.second).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按固定值计算。"),
  cancel: Schema6.natural().role("ms").default(0).description("调用 `session.cancelQueued()` 时默认的延迟。"),
  broadcast: Schema6.natural().role("ms").default(0.5 * Time3.second).description("调用 `bot.broadcast()` 时默认的延迟。"),
  prompt: Schema6.natural().role("ms").default(Time3.minute).description("调用 `session.prompt()` 时默认的等待时间。")
}));
defineProperty7(Context.Config, "Advanced", Schema6.object({
  maxListeners: Schema6.natural().default(64).description("每种监听器的最大数量。如果超过这个数量，Koishi 会认定为发生了内存泄漏，将产生一个警告。")
}).description("高级设置"));
Context.Config.list.push(Context.Config.Basic);
Context.Config.list.push(Schema6.object({
  i18n: I18n.Config
}));
Context.Config.list.push(Schema6.object({
