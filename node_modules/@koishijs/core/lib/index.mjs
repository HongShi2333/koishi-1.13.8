...
import { Quester, Schema as Schema5 } from "@satorijs/core";
...
import { defineProperty as defineProperty4, Time as Time2 } from "cosmokit";
...
import { defineProperty as defineProperty3, Time } from "cosmokit";
  ...
  constructor(ctx, config = {}) {
    ...
    ctx.on("interaction/command", (session) => {
      if (session.event?.argv) {
        ...
        defineProperty4(session, "argv", ctx.bail("before-parse", session.content, session));
        if (!session.argv) {
    ...
    ctx.before("attach", (session) => {
      ...
      defineProperty4(session, "argv", ctx.bail("before-parse", content, session));
      if (!session.argv)
        return;
      session.argv.root = true;
      session.argv.session = session;
...
import { defineProperty as defineProperty5, remove as remove3 } from "cosmokit";
...
// src/context.ts
import { Adapter as Adapter2, Bot as Bot3, Element, h as h6, Logger as Logger5, MessageEncoder, Messenger, Quester as Quester2, Schema as Schema6, segment, Universal as Universal5, z } from "@satorijs/core";
