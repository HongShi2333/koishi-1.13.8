...
// external/webui/plugins/console/src/node/locales/zh-CN.yml
var require_zh_CN = __commonJS({
  "external/webui/plugins/console/src/node/locales/zh-CN.yml"(exports, module2) {
...
// external/webui/plugins/console/src/node/index.ts
var node_exports = {};
...
var import_meta = {};
var _NodeConsole = class _NodeConsole extends import_console.Console {
  ...
  async start() {
    ...
    this.ctx.on("server/ready", () => {
      ...
      if (this.config.open && !((_a = this.ctx.get("loader")) == null ? void 0 : _a.envData.clientCount) && !process.env.KOISHI_AGENT) {
  ...
  async transformHtml(template) {
    ...
    for (const { tag, attrs = {}, content } of head) {
      const attrString = Object.entries(attrs).map(([key, value]) => ` ${key}="${import_koishi.h.escape(value != null ? value : "", true)}"`).join("");
      headInjection += `<${tag}${attrString}>${content != null ? content : ""}</${tag}>`;
    }
...
__name(_NodeConsole, "NodeConsole");
__publicField(_NodeConsole, "inject", ["server"]);
var NodeConsole = _NodeConsole;
((NodeConsole2) => {
