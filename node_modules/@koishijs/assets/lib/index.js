...
class Assets extends koishi_1.Service {
    ctx;
    config;
    static [koishi_1.Service.provide] = 'assets';
    static filter = false;
    ...
    async transform(content) {
        return await koishi_1.h.transformAsync(content, Object.fromEntries(this.types.map((type) => {
            return [type, async (data) => {
                    if (this.config.whitelist.some(prefix => data.url.startsWith(prefix))) {
                        return (0, koishi_1.h)(type, data);
                    }
                    else {
                        return (0, koishi_1.h)(type, { url: await this.upload(data.url, data.file) });
                    }
                }];
        })));
    }
