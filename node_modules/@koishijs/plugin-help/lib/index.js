...
// koishi/plugins/common/help/src/locales/zh-CN.yml
...
// koishi/plugins/common/help/src/locales/en-US.yml
...
function apply(ctx, config) {
  ...
  function findCommand(target, session) {
    ...
    if (command == null ? void 0 : command.ctx.filter(session))
      return command;
    const data = ctx.i18n.find("commands.(name).shortcuts.(variant)", target).map((item) => ({ ...item, command: $.resolve(item.data.name) })).filter((item) => {
      var _a;
      return (_a = item.command) == null ? void 0 : _a.match(session);
    });
  ...
  const cmd = ctx.command("help [command:string]", { authority: 0, ...config }).userFields(["authority"]).userFields(createCollector("user")).channelFields(createCollector("channel")).option("showHidden", "-H").action(async ({ session, options }, target) => {
    var _a;
    if (!target) {
      const prefix = (_a = session.resolve(session.app.config.prefix)[0]) != null ? _a : "";
...
function getOptions(command, session, config) {
  ...
  Object.values(command._options).forEach((option) => {
    function pushOption(option2, name2) {
      ...
      const description = session.text((_a = option2.descPath) != null ? _a : [`commands.${command.name}.options.${name2}`, ""], option2.params);
      if (description)
        line += "  " + description;
