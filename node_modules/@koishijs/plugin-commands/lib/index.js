...
// external/webui/plugins/commands/src/index.ts
var src_exports = {};
...
// external/webui/plugins/commands/src/locales/zh-CN.yml
...
// external/webui/plugins/commands/src/command.ts
...
var _CommandManager = class _CommandManager {
  constructor(ctx, config) {
    ...
    ctx.on("command-removed", (cmd) => {
      ...
      for (const command of cmd.children) {
        const parent = (_a = this.snapshots[command.name]) == null ? void 0 : _a.parent;
  ...
  init(command) {
    ...
    if (!this.config[command.name])
      return;
    (_a = this._tasks)[_b = command.name] || (_a[_b] = this.ctx.setTimeout(() => {
    ...
    return (_a = this.snapshots)[_b = command.name] || (_a[_b] = {
      create,
      command,
  ...
  teleport(command, name, write = false) {
    ...
    if (write) {
      (_a = this.config)[_b = command.name] || (_a[_b] = {});
  ...
  alias(command, aliases, write = false) {
    ...
    if (write) {
      (_a = this.config)[_b = command.name] || (_a[_b] = {});
      this.config[command.name].name = `${((_c = command.parent) == null ? void 0 : _c.name) || ""}/${command.displayName}`;
      this.config[command.name].aliases = (0, import_koishi.filterKeys)(aliases, (key, value) => {
        return !(0, import_koishi.deepEqual)(initial.aliases[key], value || null);
      });
      this.write(command);
    }
  }
  ...
  remove(name) {
    ...
    for (const child of commands) {
      ...
      this.config[child.name].name = `${(parent == null ? void 0 : parent.name) || ""}/${child.displayName}`;
    }
  ...
  write(...commands) {
    ...
    for (const command of commands) {
      ...
      if (override.name) {
        const initial = (((_a = snapshot.parent) == null ? void 0 : _a.name) || "") + "/" + command.name;
  ...
  installWebUI() {
    this.ctx.inject(["console"], (ctx) => {
      ...
      this.entry = ctx.console.addEntry(process.env.KOISHI_BASE ? [
        ...
        return this._cache || (this._cache = Object.fromEntries(ctx.$commander._commandList.map((command) => {
          var _a, _b, _c, _d;
          return [command.name, {
            name: command.name,
            children: command.children.map((child) => child.name),
            create: (_a = this.snapshots[command.name]) == null ? void 0 : _a.create,
            initial: ((_b = this.snapshots[command.name]) == null ? void 0 : _b.initial) || { aliases: command._aliases, config: command.config, options: command._options },
            override: ((_c = this.snapshots[command.name]) == null ? void 0 : _c.override) || { aliases: command._aliases, config: null, options: {} },
            paths: ((_d = this.ctx.get("loader")) == null ? void 0 : _d.paths(command.ctx.scope)) || []
          }];
        })));
      });
...
__publicField(_CommandManager, "filter", false);
__publicField(_CommandManager, "schema", import_koishi.Schema.dict(Config).hidden());
