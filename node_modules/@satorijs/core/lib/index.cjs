...
var import_undios = require("undios");
var file = __toESM(require("undios-file"), 1);
var import_element3 = __toESM(require("@satorijs/element"), 1);
__reExport(src_exports, require("cordis"), module.exports);
var file2 = __toESM(require("undios-file"), 1);
...
((Adapter2) => {
  Adapter2.WsClientConfig = import_cordis.z.object({
    retryTimes: import_cordis.z.natural().description("初次连接时的最大重试次数。").default(6),
    retryInterval: import_cordis.z.natural().role("ms").description("初次连接时的重试时间间隔。").default(5 * import_cosmokit3.Time.second),
    retryLazy: import_cordis.z.natural().role("ms").description("连接关闭后的重试时间间隔。").default(import_cosmokit3.Time.minute)
  }).description("连接设置");
...
(0, import_cosmokit4.defineProperty)(import_undios.HTTP, "Config", import_cordis2.z.object({
  timeout: import_cordis2.z.natural().role("ms").description("等待连接建立的最长时间。"),
  proxyAgent: import_cordis2.z.string().description("使用的代理服务器地址。"),
  keepAlive: import_cordis2.z.boolean().description("是否保持连接。")
}).description("请求设置"));
import_undios.HTTP.createConfig = /* @__PURE__ */ __name(function createConfig(endpoint) {
  return import_cordis2.z.object({
    endpoint: import_cordis2.z.string().role("link").description("要连接的服务器地址。").default(typeof endpoint === "string" ? endpoint : null).required(typeof endpoint === "boolean" ? endpoint : false),
    headers: import_cordis2.z.dict(String).role("table").description("要附加的额外请求头。"),
    ...this.Config.dict
  }).description("请求设置");
}, "createConfig");
var Context = class extends cordis.Context {
  ...
  constructor(config = {}) {
    ...
    this.plugin(import_undios.HTTP, config.request);
    this.plugin(file);
  }
...
((Context3) => {
  Context3.Config = import_cordis2.z.intersect([
    import_cordis2.z.object({})
  ]);
})(Context || (Context = {}));
var Service2 = class extends cordis.Service {
  static {
    __name(this, "Service");
  }
