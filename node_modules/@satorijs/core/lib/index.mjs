...
import * as Satori from "@satorijs/protocol";
...
((Adapter2) => {
  Adapter2.WsClientConfig = Schema.object({
    ...
    async start() {
      ...
      const reconnect = /* @__PURE__ */ __name(async (initial = false) => {
        ...
        socket.addEventListener("close", ({ code, reason }) => {
          ...
          logger2.warn(`${message}, will retry in ${Time.format(timeout)}...`);
          setTimeout(() => {
            if (this.getActive())
              reconnect();
          }, timeout);
  ...
  class WsClient extends WsClientBase {
    ...
    static reusable = true;
    static Config = Adapter2.WsClientConfig;
    getActive() {
...
// satori/packages/core/src/index.ts
h3.warn = new logger.Logger("element").warn;
defineProperty2(Quester, "Config", Schema2.object({
  timeout: Schema2.natural().role("ms").description("等待连接建立的最长时间。"),
  proxyAgent: Schema2.string().description("使用的代理服务器地址。"),
  keepAlive: Schema2.boolean().description("是否保持连接。")
}).description("请求设置"));
Quester.createConfig = /* @__PURE__ */ __name(function createConfig(endpoint) {
  return Schema2.object({
    endpoint: Schema2.string().role("link").description("要连接的服务器地址。").default(typeof endpoint === "string" ? endpoint : null).required(typeof endpoint === "boolean" ? endpoint : false),
    headers: Schema2.dict(String).role("table").description("要附加的额外请求头。"),
    ...this.Config.dict
  }).description("请求设置");
}, "createConfig");
