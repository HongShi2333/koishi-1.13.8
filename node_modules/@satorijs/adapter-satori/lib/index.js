...
var SatoriBot = class extends import_satori.Bot {
  static {
  ...
  SatoriBot.prototype[method.name] = async function(...args) {
    ...
    for (const [index, field] of method.fields.entries()) {
      if (method.name === "createMessage" && field.name === "content") {
        ...
        session.elements = await session.transform(import_satori.h.normalize(args[index]));
        if (await session.app.serial(session, "before-send", session, args[3] ?? {}))
          return;
        payload[field.name] = session.elements.join("");
      } else {
        payload[field.name] = transformKey(args[index], import_satori.snakeCase);
      }
    }
...
// src/ws.ts
var SatoriAdapter = class extends import_satori2.Adapter.WsClientBase {
  constructor(ctx, config) {
    super(ctx, config);
    this.ctx = ctx;
    this.config = config;
