...
async function handleUpdate(update, bot) {
  ...
  const isBotCommand = update.message && ((_a = update.message.entities) == null ? void 0 : _a[0].type) === "bot_command";
  const code = (_b = message == null ? void 0 : message.from) == null ? void 0 : _b.language_code;
  if (code) {
    if (code === "zh-hans") {
  ...
  if (isBotCommand) {
    ...
    if (data.chat.type === "private") {
      session.event.channel = {
        ...
        type: import_satori.Universal.Channel.Type.DIRECT
      };
    } else {
      session.event.guild = {
        id: data.chat.id.toString(),
...
async function decodeMessage(bot, data, message, payload = message) {
    ...
    for (const e of entities) {
      ...
      if (e.type === "mention") {
        ...
        if (eText === "@" + bot.user.name) {
          segs.push((0, import_satori.h)("at", { id: bot.user.id, name: atName }));
        } else {
          segs.push((0, import_satori.h)("text", { content: eText }));
        }
      } else if (e.type === "text_mention") {
        segs.push((0, import_satori.h)("at", { id: e.user.id }));
      } else {
        segs.push((0, import_satori.h)("text", { content: eText }));
      }
      if (e.offset > curr) {
        segs.splice(-1, 0, (0, import_satori.h)("text", { content: text.slice(curr, e.offset) }));
      }
      curr = e.offset + e.length;
    }
    if (curr < (text == null ? void 0 : text.length) || 0) {
      segs.push((0, import_satori.h)("text", { content: text.slice(curr) }));
    }
    return segs;
  }, "parseText");
  const segments = [];
  ...
  if (data.caption) {
    segments.push((0, import_satori.h)("text", { content: " " }));
  }
  const addResource = /* @__PURE__ */ __name(async (type, data2) => {
    ...
    segments.push((0, import_satori.h)(type, attrs));
  }, "addResource");
  if (data.location) {
    segments.push((0, import_satori.h)("location", { lat: data.location.latitude, lon: data.location.longitude }));
  } else if (data.photo) {
    const photo = data.photo.sort((s1, s2) => s2.file_size - s1.file_size)[0];
    segments.push((0, import_satori.h)("img", await bot.$getFileFromId(photo.file_id)));
    ...
    try {
      ...
      segments.push((0, import_satori.h)("img", await bot.$getFileFromPath(file.file_path)));
    } catch (e) {
      bot.logger.warn("get file error", e);
      segments.push((0, import_satori.h)("text", { content: `[${data.sticker.set_name || "sticker"} ${data.sticker.emoji || ""}]` }));
    }
  } else if (data.voice) {
...
async function appendAsset(bot, form, element) {
  let method;
...
__name(appendAsset, "appendAsset");
var supportedElements = ["b", "strong", "i", "em", "u", "ins", "s", "del", "a"];
var _TelegramMessageEncoder = class _TelegramMessageEncoder extends import_satori2.MessageEncoder {
  asset = null;
  payload;
  mode = "default";
  rows = [];
  ...
  async sendAsset() {
    ...
    const result = await this.bot.internal[method](form);
  ...
  async visit(element) {
    ...
    if (type === "text") {
      this.payload.caption += import_satori2.h.escape(attrs.content);
    } else if (type === "br") {
      this.payload.caption += "\n";
    } else if (type === "p") {
      ...
      const { lang } = attrs;
      this.payload.caption += `<code${lang ? ` class="language-${lang}"` : ""}>${import_satori2.h.escape(attrs.content)}</code>`;
    } else if (type === "at") {
      if (attrs.id) {
...
__name(_TelegramMessageEncoder, "TelegramMessageEncoder");
var TelegramMessageEncoder = _TelegramMessageEncoder;

// satori/adapters/telegram/src/server.ts
var import_satori3 = require("@satorijs/satori");
var _HttpServer = class _HttpServer extends import_satori3.Adapter {
  async connect(bot) {
    let { token, path, selfUrl } = bot.config;
    path = (0, import_satori3.sanitize)(path || "/telegram");
    if (selfUrl) {
      selfUrl = (0, import_satori3.trimSlash)(selfUrl);
    } else {
...
var _HttpPolling = class _HttpPolling extends import_satori4.Adapter {
  offset = 0;
  timeout;
  async connect(bot) {
      ...
      const polling = /* @__PURE__ */ __name(async () => {
        ...
        try {
          ...
          if (_initial && _retryCount > retryTimes) {
            bot.error = e;
            return bot.status = import_satori4.Universal.Status.OFFLINE;
          }
          if (!bot.isActive)
            return;
          _retryCount++;
          bot.status = import_satori4.Universal.Status.RECONNECT;
...
__name(_HttpPolling, "HttpPolling");
__publicField(_HttpPolling, "reusable", true);
var HttpPolling = _HttpPolling;
((HttpPolling2) => {
  HttpPolling2.Config = import_satori4.Schema.object({
    protocol: import_satori4.Schema.const("polling").required(process.env.KOISHI_ENV !== "browser"),
    pollingTimeout: import_satori4.Schema.natural().role("ms").default(import_satori4.Time.second * 25).description("通过长轮询获取更新时请求的超时 (单位为毫秒)。"),
...
var _SenderError = class _SenderError extends Error {
...
__name(_SenderError, "SenderError");
var SenderError = _SenderError;
var _TelegramBot = class _TelegramBot extends import_satori5.Bot {
  http;
  file;
  internal;
  local;
  ...
  constructor(ctx, config) {
    ...
    const selfUrl = config["selfUrl"] || ((_a = ctx.get("server")) == null ? void 0 : _a.config.selfUrl);
    if ((_b = config.files.server) != null ? _b : selfUrl) {
      const route = `/telegram/${this.selfId}`;
      this.server = selfUrl + route;
      ctx.get("server").get(route + "/:file+", async (ctx2) => {
        const { data, mime } = await this.$getFile(ctx2.params.file);
  ...
  async editMessage(chat_id, message_id, content) {
    ...
    payload.text = import_satori5.h.normalize(content).join("");
  ...
  async $getFileFromPath(filePath) {
    ...
    if (mime === "application/octet-stream") {
      mime = (_a = await import_file_type.default.fromBuffer(data)) == null ? void 0 : _a.mime;
    }
    const base64 = `data:${mime};base64,` + (0, import_satori5.arrayBufferToBase64)(data);
    return { src: base64 };
  }
  ...
  async getUser(userId, guildId) {
    ...
    if (!((_a = data.photo) == null ? void 0 : _a.big_file_id) && !((_b = data.photo) == null ? void 0 : _b.small_file_id))
      return decodeUser(data);
    const { src } = await this.$getFileFromId(((_c = data.photo) == null ? void 0 : _c.big_file_id) || ((_d = data.photo) == null ? void 0 : _d.small_file_id));
    return {
      ...decodeUser(data),
      avatar: src
    };
  ...
  async createDirectChannel(id) {
    return { id, type: import_satori5.Universal.Channel.Type.DIRECT };
  }
  async updateCommands(commands) {
    ...
    for (const cmd of commands) {
      ...
      for (const locale in description) {
        ...
        languages[lang] || (languages[lang] = description[locale]);
      }
      for (const lang in languages) {
        (_a = result[lang]) != null ? _a : result[lang] = [];
...
__name(_TelegramBot, "TelegramBot");
__publicField(_TelegramBot, "MessageEncoder", TelegramMessageEncoder);
__publicField(_TelegramBot, "inject", ["http"]);
...
((TelegramBot2) => {
  TelegramBot2.Config = import_satori5.Schema.intersect([
    import_satori5.Schema.object({
      ...
      protocol: process.env.KOISHI_ENV === "browser" ? import_satori5.Schema.const("polling").default("polling") : import_satori5.Schema.union(["server", "polling"]).description("选择要使用的协议。").required()
    }),
