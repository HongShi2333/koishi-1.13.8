...
var Builder = class {
  ...
  parseSelection(sel) {
    ...
    if (!sel.args[0].$) {
      ...
      return `(ifnull((SELECT ${this.groupArray(this.transform(output, import_minato.Type.getInner(import_minato.Type.fromTerm(expr)), "encode"))}
  ...
  encode(value, encoded, pure = false, type) {
    return this.asEncoded(encoded === this.isEncoded() && !pure ? value : encoded ? `cast(${this.transform(value, type, "encode")} as json)` : `json_unquote(${this.transform(value, type, "decode")})`, pure ? void 0 : encoded);
  }
  isEncoded(key) {
  ...
  groupObject(_fields) {
    ...
    return this.asEncoded(_groupObject((0, import_minato.unravel)(_fields), this.state.type, ""), true);
  }
  groupArray(value) {
  ...
  parseEvalExpr(expr) {
    ...
    for (const key in expr) {
      if (key in this.evalOperators) {
        this.state.type = import_minato.Type.fromTerm(expr);
  ...
  get(sel, inline = false, group = false, addref = true) {
    ...
    const encodedMap = {};
    const fields = args[0].fields ?? Object.fromEntries(Object.entries(model.fields).filter(([, field]) => !field.deprecated).map(([key, field]) => [key, field.expr ? field.expr : (0, import_minato.Eval)("", [ref, key], import_minato.Type.fromField(field))]));
  ...
  dump(value, type, root = true) {
    ...
    if (import_minato.Type.isType(type) || (0, import_minato.isEvalExpr)(type)) {
      ...
      if (type.inner || type.type === "json")
        root = false;
      let res = value;
      if (!(0, import_cosmokit.isNullable)(res) && type.inner) {
        if (import_minato.Type.isArray(type)) {
          res = res.map((x) => this.dump(x, import_minato.Type.getInner(type), root));
        } else {
          res = (0, import_cosmokit.mapValues)(res, (x, k) => this.dump(x, import_minato.Type.getInner(type, k), root));
        }
      }
  ...
  load(value, type, root = true) {
    ...
    if (import_minato.Type.isType(type) || (0, import_minato.isEvalExpr)(type)) {
      ...
      if (!(0, import_cosmokit.isNullable)(res) && type.inner) {
        if (import_minato.Type.isArray(type)) {
          res = res.map((x) => this.load(x, import_minato.Type.getInner(type), false));
        } else {
          res = (0, import_cosmokit.mapValues)(res, (x, k) => this.load(x, import_minato.Type.getInner(type, k), false));
        }
      }
