...
import { Eval, Field, isComparable, isEvalExpr, randomId, Selection, Type, unravel } from "minato";
...
var Builder = class {
  ...
  get(sel, inline = false, group = false, addref = true) {
    ...
    const encodedMap = {};
    const fields = args[0].fields ?? Object.fromEntries(Object.entries(model.fields).filter(([, field]) => !field.deprecated).map(([key, field]) => [key, field.expr ? field.expr : Eval("", [ref, key], Type.fromField(field))]));
    ...
    if (Type.isType(type) || isEvalExpr(type)) {
      ...
      if (!isNullable(res) && type.inner) {
        if (Type.isArray(type)) {
          res = res.map((x) => this.dump(x, Type.getInner(type), root));
        } else {
          res = mapValues(res, (x, k) => this.dump(x, Type.getInner(type, k), root));
        }
      }
  ...
  load(value, type, root = true) {
    ...
    if (Type.isType(type) || isEvalExpr(type)) {
      ...
      if (!isNullable(res) && type.inner) {
        if (Type.isArray(type)) {
          res = res.map((x) => this.load(x, Type.getInner(type), false));
        } else {
          res = mapValues(res, (x, k) => this.load(x, Type.getInner(type, k), false));
        }
      }
