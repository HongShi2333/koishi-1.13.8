...
// src/database.ts
import { makeArray as makeArray2, valueMap as valueMap3 } from "cosmokit";
import { Service } from "cordis";

// src/selection.ts
import { defineProperty as defineProperty2, valueMap as valueMap2 } from "cosmokit";
...
var Eval = /* @__PURE__ */ __name((key, value) => defineProperty({ ["$" + key]: value }, kExpr, true), "Eval");
var operators = {};
...
defineProperty(Eval, "length", unary("length", (expr, table) => Array.isArray(table) ? table.map((data) => executeAggr(expr, data)).length : Array.from(executeEval(table, expr)).length));
...
var Executable = class {
  ...
  constructor(driver, payload) {
    ...
    defineProperty2(this, "driver", driver);
    defineProperty2(this, "model", driver.model(this.table));
    defineProperty2(this, "row", createRow(this.ref, {}, "", this.model));
  }
  resolveQuery(query = {}) {
  ...
  resolveFields(fields) {
    ...
    if (Array.isArray(fields)) {
      ...
      return valueMap2(fields, (field) => this.resolveField(field));
    }
  }
  async execute() {
...
// src/model.ts
import { clone, isNullable as isNullable2, makeArray } from "cosmokit";
...
var Database = class extends Service {
  static {
  ...
  join(tables, query, optional) {
    if (Array.isArray(tables)) {
      ...
      const sel = new Selection(this.getDriver(Object.values(tables)[0]), valueMap3(tables, (t) => typeof t === "string" ? this.select(t) : t));
      if (typeof query === "function") {
...
// src/driver.ts
import { valueMap as valueMap4 } from "cosmokit";
var Driver = class {
  ...
  model(table) {
    ...
    if (table instanceof Selection) {
      ...
      model2.fields = valueMap4(table.args[0].fields, (_, key) => ({
        type: "expr"
      }));
      return model2;
    }
...
var queryOperators = {
  ...
  // existence
  $exists: (query, data) => query !== isNullable3(data),
  // comparison
...
function executeFieldQuery(query, data) {
  ...
  } else if (isNullable3(query)) {
    return isNullable3(data);
  }
...
// src/index.ts
import { Logger, Schema, Schema as Schema2 } from "cordis";
