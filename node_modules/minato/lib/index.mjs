...
// src/database.ts
import { defineProperty as defineProperty4, makeArray as makeArray2, mapValues as mapValues4, omit } from "cosmokit";
...
// src/selection.ts
import { defineProperty as defineProperty3, filterKeys } from "cosmokit";
...
// src/model.ts
import { Binary as Binary2, clone, isNullable as isNullable2, makeArray, mapValues as mapValues2 } from "cosmokit";
  ...
  parse(source, strict = true, prefix = "", result = {}) {
    ...
    for (const key in source) {
      ...
      if (key in source) {
        ...
        } else if (!value || typeof value !== "object" || isEvalExpr(value) || Array.isArray(value) || Binary2.is(value) || Object.keys(value).length === 0) {
          if (strict) {
...
var Eval3 = /* @__PURE__ */ __name((key, value, type) => defineProperty2(defineProperty2({ ["$" + key]: value }, kExpr, true), Type.kType, type), "Eval");
...
defineProperty2(Eval3, "length", unary("length", (expr, table) => Array.isArray(table) ? table.map((data) => executeAggr(expr, data)).length : Array.from(executeEval(table, expr)).length, Type.Number));
...
var Database = class _Database extends Service {
  ...
  extend(name, fields, config = {}) {
    ...
    if (makeArray2(model.primary).every((key) => key in fields)) {
      defineProperty4(model, "ctx", this[Context.origin]);
    }
    this.prepareTasks[name] = this.prepare(name);
  ...
  _parseField(field, transformers = [], setInitial, setField) {
    if (field === "object") {
      ...
      const res = Type.Object(mapValues4(inner, (x, k) => this.parseField(x, transformers, (value) => initial[k] = value)));
...
// src/driver.ts
import { mapValues as mapValues5, remove } from "cosmokit";
  ...
  model(table) {
    ...
    if (table instanceof Selection) {
      ...
      if (table.args[0].fields) {
        model2.fields = mapValues5(table.args[0].fields, (expr) => ({
          type: Type.fromTerm(expr)
        }));
      } else {
        model2.fields = mapValues5(table.model.fields, (field) => ({
          type: Type.fromField(field)
        }));
      }
      return model2;
...
// src/query.ts
import { isNullable as isNullable4 } from "cosmokit";
var queryOperators = {
  // logical
  ...
  // existence
  $exists: (query, data) => query !== isNullable4(data),
  // comparison
...
function executeFieldQuery(query, data) {
  ...
  } else if (isNullable4(query)) {
    return isNullable4(data);
  }
