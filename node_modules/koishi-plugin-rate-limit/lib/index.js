...
// external/common/packages/rate-limit/src/index.ts
var src_exports = {};
...
// external/common/packages/rate-limit/src/admin.ts
var admin_exports = {};
...
function apply(ctx) {
  ...
  ctx.command("timer [key] [value:date]").userFields(["timers"]).option("set", "-s", { authority: 4 }).option("clear", "-c", { authority: 4 }).action(({ session, options }, name3, value) => {
    ...
    for (const name4 of Object.keys(user.timers).sort()) {
      if (name4.startsWith("_"))
        continue;
      output.push(session.text(".item", [name4, user.timers[name4] - now]));
    }
    if (!output.length)
      return session.text(".none");
    output.unshift(session.text(".list"));
...
// external/common/packages/rate-limit/src/locales/zh-CN.yml
...
function apply2(ctx) {
  ...
  ctx.schema.extend("command", import_koishi.Schema.object({
    ...
    maxUsage: import_koishi.Schema.computed(import_koishi.Schema.number(), { userFields: ["authority"] }).description("每天的调用次数上限。"),
    minInterval: import_koishi.Schema.computed(import_koishi.Schema.number(), { userFields: ["authority"] }).description("连续调用的最小间隔。")
  }), 800);
  ctx.schema.extend("command-option", import_koishi.Schema.object({
    notUsage: import_koishi.Schema.boolean().description("不计入调用次数。")
  }), 800);
  ...
  ctx.before("command/execute", (argv) => {
    ...
    if (maxUsage < Infinity && checkUsage(name3, session.user, maxUsage)) {
  ...
  ctx.on("help/command", (output, command, session) => {
    ...
    const name3 = getUsageName(command);
    const maxUsage = (_a = session.resolve(command.config.maxUsage)) != null ? _a : Infinity;
    const minInterval = (_b = session.resolve(command.config.minInterval)) != null ? _b : 0;
