...
var name = "undios-proxy-agent";
var inject = ["http"];
var Config = import_cordis.z.object({});
function apply(ctx, config) {
  ctx.on("http/fetch-init", (url, init, config2) => {
    if (!config2?.proxyAgent)
      return;
    const proxy = new URL(config2.proxyAgent);
  ...
  ctx.on("http/websocket-init", (url, init, config2) => {
    if (!config2?.proxyAgent)
      return;
    const proxy = new URL(config2.proxyAgent);
